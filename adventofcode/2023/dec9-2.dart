...
void main() {
  File file = new File(
      'C:\\Users\\Gebruiker\\development\\PortableVSCode\\data\\code\\leetcode\\adventofcode\\2023\\dec9.txt');
  ...
  for (List<List<int>> inputCase in input) {
    print(inputCase[0].first);
    toReturn += inputCase[0].first;
  }
  print(toReturn);
}
...
void extrapolateLine(List<List<int>> inputCase) {
  ...
  for (int i = inputCase.length - 2; i >= 0; i--) {
    inputCase[i].insert(0, inputCase[i].first - inputCase[i + 1].first);
  }
}
