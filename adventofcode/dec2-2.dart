...
void main() {
  ...
  for (String game in input) {
    ...
    for (String round in gameContent) {
      round = round.trim();
      for (String grab in round.split(", ")) {
        List<String> split = grab.split(" ");
        switch (split[1]) {
          case "green":
            if (int.parse(split[0]) > minGreen) {
              minGreen = int.parse(split[0]);
            }
            break;
          case "red":
            if (int.parse(split[0]) > minRed) {
              minRed = int.parse(split[0]);
            }
            break;
          case "blue":
            if (int.parse(split[0]) > minBlue) {
              minBlue = int.parse(split[0]);
            }
            break;
          default:
            print("Dont know this one, ${split[1]}");
        }
      }
    }
    toreturn += (minRed * minGreen * minBlue);
  }
  print("ans: $toreturn");
}
